...
class TestQueryEndpoint:
    ...
    def test_query_qdrant_failure(self, client, mock_qdrant):
        ...
        assert response.status_code == 500
        assert "search failed" in response.json()["detail"].lower() or "query failed" in response.json()["detail"].lower()
...
class TestIntegration:
    ...
    def test_full_upsert_query_flow(self, client):
        ...
        upsert_payload = {
            "points": [
                {"id": 9000, "vector": [0.9] * 384, "payload": {"tag": "integration_test"}},
                {"id": 9001, "vector": [0.8] * 384, "payload": {"tag": "integration_test"}},
            ]
        }
